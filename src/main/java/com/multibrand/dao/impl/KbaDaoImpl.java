...
public boolean addKbaDetails(KbaQuestionResponse kbaQuestionResponse)
		throws Exception {
		...
		String reasonCodeList = StringUtils.EMPTY;
			...
			if (kbaQuestionResponse.getQuestionList() != null && kbaQuestionResponse.getQuestionList().length>0){
				...
				result = getJdbcTemplate().update(addKbaDetailsQuery,
						kbaQuestionResponse.getTransactionKey(),
					StringUtils.EMPTY,
					kbaQuestionResponse.getQuestionList().length,
					StringUtils.EMPTY,
					StringUtils.EMPTY,
					StringUtils.EMPTY,
					StringUtils.EMPTY,
					kbaQuestionResponse.getReturnCode(),
					kbaQuestionResponse.getReturnMessage(),
					errorCodeList,
					errorMsgList,
					StringUtils.EMPTY
				 );
		}else if( (kbaQuestionResponse.getQuestionList() != null || kbaQuestionResponse.getQuestionList().length==0)
					&& null!=kbaQuestionResponse.getKbaSubmitAnswerResponseOutput() 
					&& (StringUtils.isNotEmpty(kbaQuestionResponse.getKbaSubmitAnswerResponseOutput().getTransactionKey())) ){
														
			 result = getJdbcTemplate().update(addKbaDetailsQuery,
					 kbaQuestionResponse.getKbaSubmitAnswerResponseOutput().getTransactionKey(),
					 kbaQuestionResponse.getKbaSubmitAnswerResponseOutput().getDecision(),
			  	"0",
...
private String getKBAReasonCodeList(KbaQuestionResponse kbaQuestionResponse){
	...
	if(kbaQuestionResponse.getKbaSubmitAnswerResponseOutput().getKbaReasonList()!= null){
		for(KbaResponseReasonDTO reasonDTO :kbaQuestionResponse.getKbaSubmitAnswerResponseOutput().getKbaReasonList() ){
