...
/*Setting DepositReasonText Empty if we have Credit Freeze or Fraud*/		
			...
			requestData.setErrorCdList(StringUtils.join(serviceLocationResponse_errorList,SYMBOL_PIPE));
	...
	public EsidInfoTdspCalendarResponse getESIDAndCalendarDates(
		...
		try {
					...
					if(StringUtils.isNotBlank(esidDo.getEsidNumber())) {
						...
						if (strESIDNumber.equalsIgnoreCase(MESID) || strESIDNumber.equalsIgnoreCase(NESID))
						{
							...
							if (MESID.equalsIgnoreCase(strESIDNumber)) {
								...
								serviceLocationResponse_errorList.add(MESID);
								serviceLocationResponse_errorList.remove(NESID);
								serviceLocationResponse_errorList.remove(NRESID);
							} else if (NESID.equalsIgnoreCase(strESIDNumber)) {
								response.setMessageText(msgSource.getMessage(MESSAGE_CODE_NESID));
								serviceLocationResponse_errorList.add(NESID);
								serviceLocationResponse_errorList.remove(MESID);
								serviceLocationResponse_errorList.remove(NRESID);
							}
							...
							response.setAvailableDates(EMPTY);
								...
								serviceLocationResponse_errorList.add(NRESID);
								serviceLocationResponse_errorList.remove(MESID);
								serviceLocationResponse_errorList.remove(NESID);
						} else {
							...
							serviceLocationResponse_errorList.remove(NESID);
							serviceLocationResponse_errorList.remove(NRESID);
							serviceLocationResponse_errorList.add(strESIDNumber);
			...
			this.updateServiceLocation(companyCode, affiliateId, trackingId, 
					serviceAddressDO, esidDo, response,esid,StringUtils.join(serviceLocationResponse_errorList,SYMBOL_PIPE));
		}
