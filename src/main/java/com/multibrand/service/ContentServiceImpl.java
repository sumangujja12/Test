 ...
 */
@Component
public class ContentServiceImpl extends BaseRestService implements ContentService {
	...
	public String getOffer(BaseContentRequest request) {
		...
		try{
			...
			returnResp = createRestTemplateAndCallService(gson.toJson(req),GET_OFFER_REST_URL,"PSZ");//PSZ = Personalization API
	...
	public String updateUserPreference(BaseContentRequest request) {
		...
		try{
			...
			Gson gson = new Gson();
			returnResp = createRestTemplateAndCallService(gson.toJson(req),USER_UPDATE_PREF_REST_URL,"PSZ");//PSZ = Personalization API
		}catch(Exception ex){
	...
	public String readComponentByItemIds(BaseContentRequest request) {
		...
		try{
			...
			long elapsedTime;
			entryTime = System.currentTimeMillis();
			returnResp = createRestTemplateAndCallService(gson.toJson(req),READ_COMPONENT_BY_ITEMIDS,"SDL");//SDL = Content API
	...
	private String replaceDynamicContentForServices(String returnResp,
			ComponentByItemIdsRequest req) throws Exception {
				...
				if (null != arr.getDynamicFields()) {
					for (String dynamicFields : arr.getDynamicFields()) {
						...
						String[] stringArray = StringUtils.split(dynamicFields,
								':');
						logger.info("key:" + stringArray[0] + " value:"
								+ stringArray[1]);
	...
	public String readMessageContent(BaseContentRequest request) {
		...
		try{
			...
			long elapsedTime;
			entryTime = System.currentTimeMillis();
			returnResp = createRestTemplateAndCallService(gson.toJson(req),READ_MESSAGE_CONTENT,"SDL");//SDL = Content API
			elapsedTime = (System.currentTimeMillis() - entryTime) / (1000);
			String elapsedTimeDisp = elapsedTime > 0 ? elapsedTime + " seconds."
					: "less than a second.";
