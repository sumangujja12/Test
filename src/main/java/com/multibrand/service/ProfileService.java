...
public class ProfileService extends BaseAbstractService {
	...
	protected ProfileDomain getProfileDomainProxy() {
		...
		return (ProfileDomain) getServiceProxy(ProfileDomainPortBindingStub.class, PROFILE_SERVICE_ENDPOINT_URL);
	}
	 ...
	 * @throws Exception
	 */
	public com.multibrand.vo.response.OfferDO getCurrentOfferDocs(String caNumber, String bpNumber, String esid,
			String contractId, String languageCode, String companyCode, com.multibrand.vo.response.OfferDO offerDO,
			String sessionId) throws Exception {

		logger.info("[Profile Service ]::::::getCurrentOfferDocs");
		// GetContractInfoResponse response = new GetContractInfoResponse();
		long startTime = CommonUtil.getStartTime();
		String request = "caNumber=" + caNumber + ",bpNumber=" + bpNumber + ",esid=" + esid + ",contractId="
				+ contractId + ",languageCode=" + languageCode;

		URL url = ZEISUGETOFFERDATAFORSWAP_Service.class.getResource("Z_E_ISU_GET_OFFERDATA_FOR_SWAP-RPM.wsdl");

		if (url == null)
			logger.info("WSDL not initialised");

		ZEISUGETOFFERDATAFORSWAP_Service port = new ZEISUGETOFFERDATAFORSWAP_Service(url);
		...
		try {
			stub.zeIsuGetOfferdataForSwap(im_caller, zeiSwapOfferInputObj, "", null, hZeiCampEnvrDetails,
					hZeiSwapOutput, hZeiOffrCDFlag);
		} catch (Exception ex) {
			if (logger.isDebugEnabled())
				logger.debug(XmlUtil.pojoToXML(request));
			utilityloggerHelper.logTransaction("getCurrentOfferDocs", false, request, ex, "",
		...
		if (zesSwapOutput.getCurrCodata() != null && zesSwapOutput.getCurrCodata().getCampaignData() != null
				&& zesSwapOutput.getCurrCodata().getCampaignData().getItem().size() > 0) {
			logger.info("[Profile Service ]::::::getCurrentOfferDocs:::inside if");
			List<ZesDocid> zesDocidList = zesSwapOutput.getCurrCodata().getCampaignData().getItem().get(0).getDocidTab()
					.getItem();
		...
		utilityloggerHelper.logTransaction("getCurrentOfferDocs", false, request, offerDO, "",
				CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
	 ...
	 * @throws Exception
	 */
	public Map<String, Object> getProfile(String accountNumber, String companyCode, String sessionId) throws Exception {
		...
		if (url == null) {
			java.util.logging.Logger.getLogger(ZEIsuGetCaProfileData_Service.class.getName()).log(
					java.util.logging.Level.INFO, "Can not initialize the default wsdl from {0}",
					"Z_E_ISU_GET_CA_PROFILE_DATA.wsdl");
		}
		...
		binding.getRequestContext().put(BindingProvider.PASSWORD_PROPERTY,
				this.envMessageReader.getMessage(CCS_PASSWORD));
		binding.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY,
				this.envMessageReader.getMessage(PROFILE_CADATA_ENDPOINT_URL_JNDINAME));
		...
		// map.put(END_POINT_URL,
		// CommonUtil.getEndpointURL(PROFILE_CADATA_ENDPOINT_URL_JNDINAME));

		try {
			stub.zeIsuGetCaProfileData(exMessage1, imCaOnly, imVkont, holderZcaOutputTt, holderZcontractOutputTt,
					exReturnCode, exSuperPartner);
		} catch (Exception ex) {
			if (logger.isDebugEnabled())
				logger.debug(XmlUtil.pojoToXML(request));
			utilityloggerHelper.logTransaction("getProfile", false, request, ex, "",
					CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
			throw ex;
			...
			if (null != zcontractOutputList && zcontractOutputList.size() > 0) {
				...
				for (ZcontractOutput contractOutput : zcontractOutput) {
					...
					if (strTouFlag != null && StringUtils.isNotBlank(strTouFlag) && strTouFlag.equalsIgnoreCase("X")) {
						responseMap.put(contractOutput.getExVertrag(),
								contractOutput.getExVertrag() + "-" + strTouFlag);
					} else {
		...
		utilityloggerHelper.logTransaction("getProfile", false, request, profileResponse,
				profileResponse.getErrorMessage(), CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
	 ...
	 */
	/**
	 * public ProfileResponse getProfile(String accountNumber) throws
	 * RemoteException {
	 * 
	 * ProfileDomain proxy = getProfileDomainProxy(); ProfileResponse response =
	 * proxy.getProfile(accountNumber);
	 * 
	 * return response; }
	 * 
	 * @throws Exception
	...
	public CrmProfileResponse getCRMProfile(CrmProfileRequest crmProfileRequest, String companyCode, String sessionId)
		...
		try {
			...
			logger.error(ex);
			utilityloggerHelper.logTransaction("getCRMProfile", false, crmProfileRequest, ex, "",
					CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
			throw ex;
		} catch (Exception ex) {
			if (logger.isDebugEnabled())
		...
		utilityloggerHelper.logTransaction("getCRMProfile", false, crmProfileRequest, response,
				response.getErrorMessage(), CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(crmProfileRequest));
	 ...
	 * @throws Exception
	 */
	public UpdateAddressResponse updateBillingAddress(UpdateAddressRequest request, String companyCode,
			String sessionId) throws Exception {
		...
		try {
			...
			logger.error(ex);
			utilityloggerHelper.logTransaction("updateBillingAddress", false, request, ex, "",
					CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
			if (logger.isDebugEnabled())
				logger.debug(XmlUtil.pojoToXML(request));
			throw ex;
		...
		utilityloggerHelper.logTransaction("updateBillingAddress", false, request, response, response.getErrorMessage(),
				CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
			logger.debug(XmlUtil.pojoToXML(response));
	 ...
	 * @throws Exception
	 */
	public UpdateContactResponse updateContactInfoWS(UpdateContactRequest request, String companyCode, String sessionId)
			throws Exception {
		ProfileDomain proxy = getProfileDomainProxy();
		long startTime = CommonUtil.getStartTime();
		...
		try {
			...
			logger.error(ex);
			utilityloggerHelper.logTransaction("updateContactInfoWS", false, request, ex, "",
					CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
			if (logger.isDebugEnabled())
				logger.debug(XmlUtil.pojoToXML(request));
			throw ex;
		...
		utilityloggerHelper.logTransaction("updateContactInfoWS", false, request, response, response.getErrorMessage(),
				CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
			logger.debug(XmlUtil.pojoToXML(response));
	...
	public UpdateContactResponse updateContactInfo(UserRegistrationRequest request, String businessPartner,
		...
		URL url = ZECrmBpContactDetailNew_Service.class.getResource("crm_bp_contact_detail_new.wsdl");
		...
		binding.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY,
				this.envMessageReader.getMessage(CONTACT_DETAIL_ENDPOINT_URL_JNDINAME));
		...
		long startTime = CommonUtil.getStartTime();
		stub.zeCrmBpContactDetailNew(imRequestType, imSubscriberId, hTZCommPref2, hTZEmailNew2, hTZPhoneNew2,
				hTBapiret2, hTZUnsubReason);
		...
		utilityloggerHelper.logTransaction("updateContactInfo", false, request, response, response.getErrorMessage(),
				CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
			logger.debug(XmlUtil.pojoToXML(response));
	...
	public GetContractInfoResponse getContractInfo(String caNumber, String bpNumber, String esid, String contractId,
			String languageCode, String companyCode, String sessionId) throws Exception {
		...
		try {
			...
			logger.error(ex);
			utilityloggerHelper.logTransaction("getContractInfo", false, zeiSwapOfferInputObj, ex, "",
					CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
			if (logger.isDebugEnabled())
		...
		if ((zesSwapOutput.getEligOffers() == null) || (zesSwapOutput.getEligOffers().getItem().size() == 0)) {
			for (com.nrg.cxfstubs.contractinfo.Bapiret2 bapiret2 : bapiRet2T) {
				if (bapiret2.getType().equalsIgnoreCase(TYPE_E) && (bapiret2.getNumber()) != null
						|| bapiret2.getMessage() != null) {
		...
		utilityloggerHelper.logTransaction("getContractInfo", false, zeiSwapOfferInputObj, response,
				response.getResultDescription(), CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(zeiSwapOfferInputObj));
			logger.debug(XmlUtil.pojoToXML(response));
		}
	...
	private GetContractInfoResponse handleContractInfoResponse(ZeiSwapOutput zeiSwapOutput,
			ZeiCampEnviDetails zeiCampEnvrDetails, ZeiOfrcdFlag zeiOfferCdFlag) {
		...
		if (zesSwapOutput.getEligOffers() != null) {
			response.setEligibleOffersList(
					populateEligibleOffers(zesSwapOutput, zesOfferCDFlagList, zesCampEnvDetailsList));
		}

		return response;
	}

	private OfferDO[] populateEligibleOffers(ZesSwapOutput zesSwapOutput, List<ZesOfrcdFlag> zesOfferCDFlagList,
			List<ZesCampEnviDetails> zesCampEnvDetailsList) {
			...
			for (com.nrg.cxfstubs.contractinfo.ZesEligoffer zesEligoffer : zesEligibleOfferList) {
				try {
					...
					List<Map<String, Object>> offerCategoryLookupDetailsList = offerService
							.getOfferCategories(zesEligoffer.getOfferCode());
					...
					eligibleOffersList[count].setStrOfferCode(zesEligoffer.getOfferCode());
					eligibleOffersList[count].setStrCampaignCode(zesEligoffer.getCampaignCd());
					eligibleOffersList[count].setStrOfferCellTrackCode(zesEligoffer.getPromoCd());
					eligibleOffersList[count].setStrTarrifType(zesEligoffer.getTariftyp());
					...
					eligibleOffersList[count]
							.setStrBundlingGroup(zesEligoffer.getCampaignData().getItem().get(0).getBundlingGroup());
					eligibleOffersList[count].setStrOfferRank("" + zesEligoffer.getPromoRank());
					eligibleOffersList[count]
							.setStrEFLIssueDate(zesEligoffer.getCampaignData().getItem().get(0).getEflIssueDate());
					eligibleOffersList[count].setStrCancelFee(
							String.valueOf(zesEligoffer.getCampaignData().getItem().get(0).getPenaltyValue()));
					eligibleOffersList[count]
							.setStrContractTerm(zesEligoffer.getCampaignData().getItem().get(0).getContractLength());
					eligibleOffersList[count]
							.setStrPlanName(zesEligoffer.getCampaignData().getItem().get(0).getPlanname().getDescrip());

					eligibleOffersList[count] = this.populatePriceAndDocTypelists(eligibleOffersList[count],
							zesEligoffer);
					...
					eligibleOffersList[count]
							.setStrTDSPCode(zesEligoffer.getCampaignData().getItem().get(0).getServiceid());
	 ...
	 * @return
	 */
	private com.multibrand.vo.response.OfferDO populatePriceAndDocTypelists(com.multibrand.vo.response.OfferDO offerDO,
	 ...
	 * @throws Exception
	 */
	public EsidProfileResponse getESIDProfile(String companyCode, String esid, String sessionId) throws Exception {
		ProfileDomain proxy = getProfileDomainProxy();
		...
		try {
			...
			logger.error(ex);
			utilityloggerHelper.logTransaction("getESIDProfile", false, request, ex, "",
					CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
			if (logger.isDebugEnabled())
				logger.debug(XmlUtil.pojoToXML(request));
			throw ex;
		...
		utilityloggerHelper.logTransaction("getESIDProfile", false, request, response, response.getErrorMessage(),
				CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
			logger.debug(XmlUtil.pojoToXML(response));
	...
	public ProductUpdateResponse productUpdate(String accountNumber, String action, String objectId, String extUi,
			String enrollType, String requestDate, String manuPartNo, String companyCode, String sessionId)
			throws Exception {

		String request = "accountNumber=" + accountNumber + ",action=" + action + ",objectId=" + objectId + ",extUi="
				+ extUi + ",entrollType=" + enrollType + ",requestDate=" + requestDate + ",manuPartNo=" + manuPartNo
		...
		binding.getRequestContext().put(BindingProvider.PASSWORD_PROPERTY,
				this.envMessageReader.getMessage(CCS_PASSWORD));
		binding.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY,
				this.envMessageReader.getMessage(CCS_CRM_PROD_UPDATE));
	 ...
	 * @throws Exception
	 */
	public EnvironmentImpactsResponse environmentImpacts(String accountNumber, String companyCode, String sessionId)
			throws Exception {
		...
		URL url = ZEISUENVIRONMENTALIMPACTS.class.getResource("Z_E_ISU_ENVIRONMENTAL_IMPACTS.wsdl");
		...
		binding.getRequestContext().put(BindingProvider.PASSWORD_PROPERTY,
				this.envMessageReader.getMessage(CCS_PASSWORD));
		binding.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY,
				this.envMessageReader.getMessage(ENVIRONMENT_IMPACT_ENDPOINT_URL_JNDINAME));
		...
		try {
			...
			logger.error(ex);
			utilityloggerHelper.logTransaction("environmentalImpacts", false, request, ex, "",
					CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
			if (logger.isDebugEnabled())
				logger.debug(XmlUtil.pojoToXML(request));
			throw ex;
		...
		utilityloggerHelper.logTransaction("environmentalImpacts", false, request, response,
				response.getResultDescription(), CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
			logger.debug(XmlUtil.pojoToXML(response));
	...
	public SecondaryNameResponse secondaryNameUpdate(SecondaryNameUpdateReqVO request, String companyCode,
			String sessionId) throws Exception {
		...
		try {
			...
			binding.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY,
					this.envMessageReader.getMessage(CCS_SECONDARY_NAME_UPDATE_JNDINAME));
			...
			if (listBapiret2 != null && listBapiret2.size() > 0) {
				for (com.nrg.cxfstubs.bprelationreadupd.Bapiret2 bapiret2 : listBapiret2) {
					...
					logger.info("Printintg Bapiret2 values..." + bapiret2.getType() + " " + bapiret2.getNumber() + " "
							+ bapiret2.getMessage());
					if (bapiret2.getType().equalsIgnoreCase(TYPE_E)
							&& (bapiret2.getNumber() != null || bapiret2.getMessage() != null)) {
						...
						if (action.equals("4")) {
							...
							utilityloggerHelper.logTransaction("secondaryNameUpdate", false, request, response,
									response.getResultDescription(), CommonUtil.getElapsedTime(startTime), "",
	 ...
	 * @throws RemoteException
	 */
	public void activateCRM(UserRegistrationRequest request, String businessPartner, String companyCode,
		...
		binding.getRequestContext().put("javax.xml.ws.security.auth.username",
				this.envMessageReader.getMessage("CCSUSERNAME"));

		binding.getRequestContext().put("javax.xml.ws.security.auth.password",
				this.envMessageReader.getMessage("CCSPASSWORD"));
		binding.getRequestContext().put("javax.xml.ws.service.endpoint.address",
				this.envMessageReader.getMessage("CCS_CRM_ACTIVATION"));
		...
		utilityloggerHelper.logTransaction("activateCRM", false, zes, "", "", CommonUtil.getElapsedTime(startTime), "",
				sessionId, companyCode);
		if (logger.isDebugEnabled())
			logger.debug(XmlUtil.pojoToXML(request));
	...
	public WseServiceResponse wsDeEnrollService(WseServiceRequest request, String companyCode, String sessionId)
		...
		long startTime = CommonUtil.getStartTime();
		WseServiceResponse wseResponse = proxy.wseEnrollDeEnrollService(request);
		utilityloggerHelper.logTransaction("wsEnrollDeEnrollService", false, request, wseResponse,
				wseResponse.getErrorMessage(), CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
	 ...
	 * @throws RemoteException
	 */
	public WseEsenseEligibilityResponse wseEsenseEligibilityStatus(AllAccountDetailsRequest request, String companyCode,
			String sessionId) throws RemoteException, Exception {
		...
		utilityloggerHelper.logTransaction("wseEsenseEligibilityStatus", false, request, wseResponse,
				wseResponse.getErrorMessage(), CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
			logger.debug(XmlUtil.pojoToXML(wseResponse));
		}
		if (wseResponse != null && (wseResponse.getWseEsenseEligibilityResponse() != null
				&& wseResponse.getWseEsenseEligibilityResponse().getWseEsenseEligibilityItem() != null)) {
	 ...
	 * @throws Exception
	 */
	public CirroStructureCallResponse getCirroStructureCall(CirroStructureCallRequest request, String companyCode,
			String sessionId) throws RemoteException, Exception {
		...
		utilityloggerHelper.logTransaction("getCirroStructureCall", false, request, callResponse,
				callResponse.getErrorMessage(), CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
	 ...
	 * @throws RemoteException
	 */
	public com.multibrand.domain.AcctValidationResponse validateAccount(AcctValidationRequest request,
			String companyCode, String sessionId) throws RemoteException {
		...
		utilityloggerHelper.logTransaction("validateAccount", false, request, callResponse,
				callResponse.getErrorMessage(), CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
			logger.debug(XmlUtil.pojoToXML(callResponse));
	 ...
	 * @throws Exception
	 */
	public LanguageUpdateResponse updateLanguage(LanguageUpdateRequest request, String companyCode, String sessionId)
			throws Exception {
		...
		try {
			...
			if (logger.isDebugEnabled())
				logger.debug(XmlUtil.pojoToXML(request));
			logger.error(ex);
			utilityloggerHelper.logTransaction("updateLanguage", false, request, ex, "",
					CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
			throw ex;
		} catch (Exception ex) {
			if (logger.isDebugEnabled())
		...
		utilityloggerHelper.logTransaction("updateLanguage", false, request, response, response.getErrorMessage(),
				CommonUtil.getElapsedTime(startTime), "", sessionId, companyCode);
		if (logger.isDebugEnabled()) {
			logger.debug(XmlUtil.pojoToXML(request));
			logger.debug(XmlUtil.pojoToXML(response));
